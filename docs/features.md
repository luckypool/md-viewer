...
Google Drive に保存された Markdown ファイルをブラウザ上で直接編集し、Drive に書き戻すことができます。
...
1. ビューア画面でセグメントコントロールの「Edit」を選択
2. CodeMirror エディタで Markdown ソースを編集
3. `⌘S` / `Ctrl+S` で保存
4. Google Drive に変更が書き戻される
...
- 編集モードは **Google Drive ファイルでのみ** 使用可能です（ローカルファイルは不可）
- 初回編集時に `drive.file` スコープでの再認証が必要になる場合があります
...
- **閲覧スコープ**: `drive.readonly`（読み取り専用）
- **編集スコープ**: `drive.file`（編集モード使用時に追加認証）
- **認証方式**: ポップアップウィンドウ
- **セッション**: ブラウザを閉じるまで有効
